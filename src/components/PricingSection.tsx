...
const PricingSection = () => {
  return (
    <section className="relative bg-surface-dark rounded-4xl p-8 lg:p-16 overflow-hidden shadow-2xl mt-8">
      {/* Background Text */}
      ...
      <div className="relative z-10 flex flex-col items-center text-center max-w-2xl mx-auto mb-16">
        <span className="inline-flex items-center rounded-full border border-primary/30 bg-primary/10 px-3 py-1 text-xs text-emerald-400 mb-6">
          Simple Pricing
        </span>
          ...
          return (
            <div
              ...
              className={`group relative flex flex-col p-8 rounded-3xl transition-colors duration-300 ${
                plan.highlighted
                  ? "bg-white/[0.08] border border-white/20 overflow-hidden shadow-2xl"
                  : "bg-white/[0.03] border border-white/10 hover:bg-white/[0.05]"
              }`}
            >
              {plan.highlighted && (
                <div className="absolute bottom-0 left-0 right-0 h-40 bg-gradient-to-t from-primary/20 to-transparent pointer-events-none" />
              )}

              <div className="mb-6 flex items-start justify-between relative z-10">
                <div className={`h-10 w-10 rounded-xl flex items-center justify-center border ${
                  plan.highlighted ? "bg-primary/20 border-primary/30" : "bg-white/10 border-white/10"
                }`}>
                  <Icon className={plan.highlighted ? "text-emerald-400" : "text-white"} size={20} />
                </div>
                {plan.badge && (
                  <span className="bg-emerald-400 text-black text-[10px] px-2 py-1 rounded uppercase tracking-wider">{plan.badge}</span>
                )}
              </div>
              ...
              <button className={`relative z-10 w-full py-3 rounded-xl text-sm mb-8 flex items-center justify-center gap-2 transition-colors ${
                plan.highlighted
                  ? "bg-primary text-white hover:bg-emerald-600 shadow-[0_0_20px_rgba(16,185,129,0.3)]"
              ...
              <div className="space-y-4 mt-auto relative z-10">
                <p className="text-xs text-gray-500 uppercase tracking-wider">{plan.label}</p>
                <ul className="space-y-3">
                  {plan.features.map((f) => (
                    <li key={f} className={`flex items-center gap-3 text-sm ${plan.highlighted ? "text-white" : "text-gray-300"}`}>
                      <CheckCircle className={plan.highlighted ? "text-emerald-400" : "text-gray-500"} size={14} />
                      {f}
                    </li>
                  ))}
                </ul>
